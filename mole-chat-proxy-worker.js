...
export default {
  async fetch(request, env) {
    ...
    let response;
    try {
      response = await env.AI.run(MODEL, { messages, tools: TOOLS, max_tokens: 400, temperature: 0.8 });
    } catch (e) {
    ...
    if (response.tool_calls && response.tool_calls.length) {
      ...
      return jsonResponse({ toolCall: { name: tc.name, args: tc.arguments || {} } });
    }

    const reply = response.response || "Hmm, je me suis pris les pattes dans un tunnel, tu peux reformuler ?";
