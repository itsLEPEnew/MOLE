...
export async function handleTracklist(url) {
  ...
  try {
    ...
    if (stored) {
      return jsonResponse({
        source: "catalogue",
        // le catalogue stocke les durées en secondes (unité native de Deezer) ; le client,
        // lui, attend des millisecondes depuis toujours (formatDuration) — sans cette
        // conversion, chaque titre s'affichait "0:00"
        tracks: stored.map((t, i) => ({
          position: t.position || i + 1,
          recordingId: `catalog-${collectionId}-${i}`,
          title: t.title,
