...
// import réservé à la route temporaire /migrate-to-supabase ci-dessous (lit l'ancien KV
// Cloudflare pendant que env.RECOS_KV pointe déjà sur Supabase, voir mole-admin-env.js) —
// à retirer avec la route une fois la migration validée en production.
import { kvGet as cfKvGet } from "./cloudflare-kv.js";

const SPOTIFY_CLIENT_ID = process.env.SPOTIFY_CLIENT_ID || "REMPLACE_PAR_TON_CLIENT_ID";
const SPOTIFY_CLIENT_SECRET = process.env.SPOTIFY_CLIENT_SECRET || "REMPLACE_PAR_TON_CLIENT_SECRET";
...
export async function handleAdminRequest(request, env) {
    ...
    try {
      ...
      // ---------- TEMPORAIRE : bascule des données Cloudflare KV -> Supabase ----------
      // Copie "queue"/"wall"/"push_subscriptions" de l'ancien stockage vers le nouveau, en
      // une seule requête authentifiée (le mot de passe admin), sans jamais faire transiter
      // les abonnements push (semi-sensibles) ailleurs que sur le serveur. Idempotente :
      // rejouable sans risque, chaque passage écrase Supabase avec l'état actuel du KV — à
      // retirer (avec l'import cfKvGet en haut du fichier) une fois la migration validée.
      if (url.pathname === "/migrate-to-supabase" && request.method === "POST") {
        const { supaKvPut } = await import("./supabase-db.js");
        const counts = {};
        for (const key of ["queue", "wall", "push_subscriptions"]) {
          const raw = await cfKvGet(key);
          const arr = raw ? JSON.parse(raw) : [];
          await supaKvPut(key, JSON.stringify(arr));
          counts[key] = arr.length;
        }
        return jsonResponse({ ok: true, migrated: counts });
      }
      ...
      if (url.pathname === "/spotify-search" && request.method === "GET") {
        const q = url.searchParams.get("q") || "";
        const { results, reason } = await spotifySearch(q);
        return jsonResponse({ results, reason });
      }
...
// equivalent du scheduled() Cloudflare, appele par le cron Vercel (api/cron/friday.js)
export async function runFridayPublish(env) {
  const result = await publishQueue(env);
  if (result.publishedCount > 0) await sendWeeklyPush(env);
  return result;
}
...
/**
 * Renvoie { results, reason } — jamais une simple liste vide.
 *
 * Toutes les pannes se ressemblaient auparavant : non configuré, identifiants refusés et
 * 403 rendaient le même tableau vide, si bien que la recherche Spotify a pu ne jamais
 * fonctionner sans que rien ne le signale. "reason" nomme la cause quand il n'y a
 * pas de résultat, pour que la panne soit lisible dans l'admin au lieu d'être muette.
 */
async function spotifySearch(q) {
  if (!q) return { results: [], reason: null };
  if (SPOTIFY_CLIENT_ID.startsWith("REMPLACE")) {
    return { results: [], reason: "Spotify non configuré (SPOTIFY_CLIENT_ID absent)" };
  }
  try {
    ...
    if (!tokenData.access_token) {
      return { results: [], reason: `Identifiants Spotify refusés (${tokenData.error || tokenRes.status})` };
    }
    ...
    if (!searchRes.ok) {
      // 403 typique : Spotify exige que le titulaire du compte propriétaire de l'app soit
      // abonné Premium pour que la Web API réponde, même sans connexion utilisateur.
      const body = await searchRes.text();
      return { results: [], reason: `Spotify a refusé la requête (${searchRes.status}) : ${body.slice(0, 160)}` };
    }
    const searchData = await searchRes.json();
    const results = ((searchData.tracks && searchData.tracks.items) || []).map((t) => ({
    ...
    return { results, reason: results.length ? null : "Aucun résultat pour cette recherche" };
  } catch (e) {
    return { results: [], reason: `Spotify injoignable : ${e}` };
  }
}
...
async function quickAddFromUrl(rawUrl, env, debug, clientApple) {
  ...
  if (q) {
    ...
    if (!spotifyUrl) {
      const { results: spotifyResults } = await spotifySearch(q);
...
async function runGifSearch(q){
  ...
  try{
    const res = await fetch("/admin/gif-search?q=" + encodeURIComponent(q));
    const data = await res.json();
...
async function lookupSharedUrl(){
  ...
  try{
    if(raw.includes("music.apple.com")){
      ...
      const res = await fetch("/admin/fetch-meta?url=" + encodeURIComponent(raw));
...
async function searchLinks(){
  ...
  try{
    const res = await fetch("/admin/spotify-search?q=" + encodeURIComponent(q));
    const data = await res.json();
    window.__spotifyResults = data.results || [];
    window.__spotifyReason = data.reason || null;
  }catch(e){ window.__spotifyResults = []; window.__spotifyReason = "Recherche Spotify injoignable"; }
...
function renderMatches(){
  renderMatchList("matchesSpotify", "Spotify", window.__spotifyResults || [], "spotify", window.__spotifyReason);
...
function renderMatchList(elId, label, results, key, reason){
  const el = $(elId);
  if(!results.length && !selected[key]){
    // une colonne vide sans explication a longtemps masqué une panne complète :
    // quand le serveur donne une raison, on l'affiche plutôt que de ne rien montrer.
    el.innerHTML = reason
      ? "<div style='font-size:11px;color:var(--ink-soft);margin-top:6px;'>" + label +
        "</div><div style='font-size:11px;color:#c2410c;margin-top:2px;'>" + reason + "</div>"
      : "";
    return;
  }
...
async function addToQueue(){
  ...
  try{
    const res = await fetch("/admin/queue", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(item) });
...
async function reorder(queue, from, to){
  ...
  await fetch("/admin/queue-reorder", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({order: ids}) });
  loadQueue();
}

async function removeItem(id){
  await fetch("/admin/queue/" + id, { method:"DELETE" });
  loadQueue();
}
...
async function backfillTracklists(){
  ...
  try{
    const res = await fetch("/admin/backfill-tracklists", { method: "POST" });
    const data = await res.json();
...
async function loadWall(){
  ...
  if(wallCache === null){
    ...
    try{
      const res = await fetch("/admin/wall");
      const data = await res.json();
      wallCache = data.wall || [];
    }catch(e){
...
$("weSaveBtn").addEventListener("click", async () => {
  ...
  try{
    await fetch("/admin/wall/" + editingWallId, { method:"PUT", headers:{"Content-Type":"application/json"}, body: JSON.stringify(patch) });
    wallCache = null;
    closeWallEdit();
    loadWall();
  }catch(e){
...
$("weDeleteBtn").addEventListener("click", async () => {
  ...
  await fetch("/admin/wall/" + editingWallId, { method:"DELETE" });
  wallCache = null;
  closeWallEdit();
  loadWall();
});
...
async function editWallStyle(id){
  ...
  await fetch("/admin/wall/" + id, { method:"PUT", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ style, substyle }) });
  wallCache = null;
  loadWall();
}

function populateStyleDatalist(){
  fetch("/admin/wall").then(r => r.json()).then(data => {
    const styles = new Set();
...
async function runGifSearch(q){
  $("gifResults").innerHTML = "<div style='font-size:12px;color:var(--ink-soft);'>Chargement...</div>";
  try{
    const res = await fetch("/admin/quick/gif-search?key=" + encodeURIComponent(KEY) + "&q=" + encodeURIComponent(q));
    const data = await res.json();
    const results = data.results || [];
    if(!results.length){
      $("gifResults").innerHTML = "<div style='font-size:12px;color:var(--ink-soft);'>Aucun résultat.</div>";
...
$("addBtn").addEventListener("click", async () => {
  ...
  try{
    const res = await fetch("/admin/quick/save", {
      method: "POST",
